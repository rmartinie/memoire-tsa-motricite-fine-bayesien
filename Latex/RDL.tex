...
%plus value motricité fine ?
%JUSTIFIER QQ PART POURQUOI FINE ET PAS GLOBALE ?
...
\subsubsection{Classification en fonction de variables motrices}
...
Face à cette hétérogénéité, l'identification des variables motrices les plus caractéristiques permettrait d'orienter la collecte de données pour le développement d'outils diagnostics basés sur la motricité. De plus, dans une perspective de modélisation "utile", cela permettrait d'inférer les systèmes neuronaux sous-jacents impliqués, puis de formuler des hypothèses sur les TSA en s'appuyant sur les connaissances associées à ces systèmes. Par exemple, si les variables les plus discriminantes reflètent principalement des processus de planification, cela suggérerait une implication prédominante des ganglions de la base et permettrait de mobiliser les connaissances issues de la maladie de Parkinson pour mieux comprendre les mécanismes autistiques \parencite{Rinehart2006Aug}.
...
\section{Hypothèses}
Sur la base de la littérature et des questionnements précédents, nous faisons l'hypothèse qu'il sera possible de classifier les enfants présentant un TSA par rapport aux enfants neurotypiques (NT) avec une précision de l'ordre de 70 à 90\,\%, à partir de l'ensemble des variables motrices issues des différentes tâches, et ce, sans surapprentissage. Nous supposons par ailleurs qu'un sous-ensemble restreint, correspondant aux variables les plus discriminantes, sera suffisant pour classifier les deux groupes avec des performances comparables à celles obtenues avec l'ensemble complet des marqueurs% (typiquement <2 sd ELPD/qq \%)
