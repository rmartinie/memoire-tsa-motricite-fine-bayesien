...
À partir des enregistrements issus des quatre tâches, 142 variables motrices ont été extraites, auxquelles s'ajoutent l'âge et le sexe de chaque enfant, soit 144 variables prédictives (\textit{features}) au total. Ces variables se répartissent en trois catégories : des variables directes? (distances, vitesses...), des variables dérivées (SPARC, curve index...) et des variables spécifiques aux tâches (indice d'isochronie, pente de la loi de Fitts...). Une description complète de l'ensemble des variables est disponible en	\hyperref[table:annexe_description_var]{Annexe A}. Le ratio variables/observations ($D/n = 144/94 \approx 1.5$) est supérieur à~1. L'analyse statistique est donc dans un cas de grande dimension + citation fléau de la dimensionnalité ?. Ce constat motive le recours à un cadre régularisant pour déterminer les variables les plus caractéristiques.
...
La performance prédictive du modèle est évaluée par validation croisée \textit{leave-one-out} (LOO-CV), approchée par l'estimateur PSIS-LOO (\textit{Pareto Smoothed Importance Sampling Leave-One-Out}) \parencite{Vehtari2015Jul}. Il estime l'\textit{expected log pointwise predictive density} (elpd), définie telle que :
